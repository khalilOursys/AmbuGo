// app/admin/[companyId]/equipment/edit/[id]/page.tsx
"use client";
...
const fetchEquipment = async (id: string): Promise<Equipment> => {
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/equipment/${id}`);
  if (!response.ok) throw new Error("Failed to fetch equipment");
  return response.json();
};

const updateEquipment = async ({ id, data }: { id: string; data: UpdateEquipmentDto }) => {
  // Convert purchasePrice to float if it exists and is a string
  const formattedData = { ...data };

  if (formattedData.purchasePrice !== undefined && formattedData.purchasePrice !== null) {
    formattedData.purchasePrice = parseFloat(formattedData.purchasePrice as any);
    if (isNaN(formattedData.purchasePrice)) {
      throw new Error("Purchase price must be a valid number");
    }
  }

  if (formattedData.quantity !== undefined && formattedData.quantity !== null) {
    formattedData.quantity = parseInt(formattedData.quantity as any);
...
export default function EditEquipmentPage() {
  ...
  const submitForm = async (event: React.FormEvent) => {
    ...
    updateMutation.mutate({ id: equipmentId, data: formData });
  };
