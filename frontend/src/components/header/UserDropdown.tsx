...
import { AuthUser, getUser, logout } from "@/lib/api/auth";
...
export default function UserDropdown() {
  ...
  // Read user from localStorage after mount (avoids SSR mismatch)
  useEffect(() => {
    setMounted(true);
    setUser(getUser());
  ...
  const handleLogout = (e: React.MouseEvent) => {
    e.preventDefault();
    closeDropdown();
    logout(); // clears localStorage + redirects to /signin
  };

  // Fallbacks while not mounted / no user
  const displayName =
    mounted && user ? `${user.firstName} ${user.lastName}` : "User";
  const displayFirstName = mounted && user ? user.firstName : "User";
